...
(async function run() {
	...
	if (config.repo && !config.projectVersion) {
		try {
			const release = await (await fetch(`https://api.github.com/repos/${config.repo}/releases`)).json();
			config.projectVersion = release[0] && release[0].tag_name;
		} catch (error) {
			/* ignore errors */
		}
	}
